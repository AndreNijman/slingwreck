...
async function initLocal(page, siege) {
  await page.evaluate(async (payload) => {
    const sim = await import('./sim.js?v=20260902-1');
    const { decode } = await import('./build.js?v=20260902-1');
    const { TUNE } = await import('./data.js?v=20260902-1');
...
async function localMiss(page, tapAbility = false) {
  return page.evaluate(async (tapAbility) => {
    const sim = await import('./sim.js?v=20260902-1');
    const { AMMO_BY_ID, TUNE } = await import('./data.js?v=20260902-1');
